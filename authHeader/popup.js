...
document.getElementById("compareButton").addEventListener("click", async (tab) => {
  ...
  await goToUrl(responseTabID[0], `https://${hostE}.o3.ru/auth/realms/${pathE}/protocol/openid-connect/auth?response_type=token&client_id=k8s.internal-headers-token&redirect_uri=https%3A%2F%2Finternal-headers-token.${pathE}.a.o3.ru%3A84%2Foauth2-redirect.html&scope=profile%20roles`)
  ...
  xmlHttp.open("GET", "https://internal-headers-token.stg.a.o3.ru:80/v1/token", false); // false for synchronous request
